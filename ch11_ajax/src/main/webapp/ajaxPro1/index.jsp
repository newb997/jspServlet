	...
	<script>
		$(() =>{
			...
			$idInput.keyup(function() {
				// 최소 3글자 이상 입력되었을때 ajax요청으로 중복체크
					...
					$.ajax({
						...
						success : function(result){
							if(result == "idN") {
								...
								$("#enrollFrm :submit").attr("disalbed",true);
							} else {
								$('#checkResult').show();
								$('#checkResult').css("color","green")
												 .text("사용 가능한 아이디 입니다");
								$("#enrollFrm :submit").attr("disalbed",false);
							}
						},
						error : function(){
...
<!-- 	
	...
	<script>
		function idCheck() {
			...
			$.ajax({
				...
				error : function(){
					console.log("아이디 중복체그 ajax통신 실패");
				}
			})
		}
	</script>
	...
	<script>
		function test2() {
			$.ajax({
				url : "ajax2.do",	// 매핑할때 /ajaxPro1/ajax2.do 라고 써주면됨
				data : {
					name : $("#input2_1").val(),
					age : $("#input2_2").val()
				},
				...
				success : function(value){
					// version2. 로 받았을 때 하나의 문자열로 들어옴 
					// console.log(value);
					
					// version3. 로 받았을 때 JSONArray로 받음
// 					console.log(value);
// 					console.log(value[0]);
// 					console.log(value[1]);

					// version4. JSONObject로 받았을 때
					console.log(value);
...
// 								+ "EMAIL : " + result.email + "<br>"
	...
	<button onclick="test4()">회원 전체조회</button><br>
